...
def train_stacked_autoencoder(X_train, X_valid, X_test, epochs=20, base_path:str="."):
    stacked_ae = models.StackedAutoencoder()
    stacked_ae.compile(optimizer=keras.optimizers.Nadam(), loss=keras.losses.Huber())
    callbacks = get_default_callbacks(model_name="stacked_ae", ckpt_path=f"{base_path}/ckpt", tensorboard_path=f"{base_path}/runs", csv_log_dir=f"{base_path}/logs")

    stacked_ae.fit(X_train, X_train, epochs=epochs,
                    validation_data=(X_valid, X_valid),
                    callbacks=callbacks)
    stacked_ae.evaluate(X_test, X_test)
    keras.models.save_model(stacked_ae, f"{base_path}/saved_models/stacked_autoencoder.keras")

def train_tied_stacked_autoencoder(X_train, X_valid, X_test, epochs=20, base_path:str="."):
    ...
    callbacks = get_default_callbacks(model_name="tied_stacked_ae", ckpt_path=f"{base_path}/ckpt", tensorboard_path=f"{base_path}/runs", csv_log_dir=f"{base_path}/logs")

    tied_stacked_ae.fit(X_train, X_train, epochs=epochs,
                    validation_data=(X_valid, X_valid),
                    callbacks=callbacks)
    tied_stacked_ae.evaluate(X_test, X_test)
    keras.models.save_model(tied_stacked_ae, f"{base_path}/saved_models/tied_stacked_autoencoder.keras")

def train_saprse_stacked_autoencoder(X_train, X_valid, X_test, epochs=20, base_path:str="."):
...
def train_convolutional_autoencoder(X_train, X_valid, X_test, epochs=20, base_path:str="."):
    conv_ae = models.ConvolutionalAutoencoder()
    conv_ae.compile(optimizer=keras.optimizers.Nadam(), loss=keras.losses.Huber())
    callbacks = get_default_callbacks(model_name="conv_ae", ckpt_path=f"{base_path}/ckpt", tensorboard_path=f"{base_path}/runs", csv_log_dir=f"{base_path}/logs")

    conv_ae.fit(X_train, X_train, epochs=epochs,
                    validation_data=(X_valid, X_valid),
                    callbacks=callbacks)
    conv_ae.evaluate(X_test, X_test)
    keras.models.save_model(conv_ae, f"{base_path}/saved_models/convolutional_autoencoder.keras")

def train_tied_convolutional_autoencoder(X_train, X_valid, X_test, epochs=20, base_path:str="."):
    ...
    callbacks = get_default_callbacks(model_name="tied_conv_ae", ckpt_path=f"{base_path}/ckpt", tensorboard_path=f"{base_path}/runs", csv_log_dir=f"{base_path}/logs")

    tied_conv_ae.fit(X_train, X_train, epochs=epochs,
                    validation_data=(X_valid, X_valid),
                    callbacks=callbacks)
    tied_conv_ae.evaluate(X_test, X_test)
    keras.models.save_model(tied_conv_ae, f"{base_path}/saved_models/tied_convolutional_autoencoder.keras")

def train_denoise_convolutional_autoencoder(X_train, X_valid, X_test, epochs=20, base_path:str="."):
    ...
    callbacks = get_default_callbacks(model_name="denoise_conv_ae", ckpt_path=f"{base_path}/ckpt", tensorboard_path=f"{base_path}/runs", csv_log_dir=f"{base_path}/logs")

    denoise_conv_ae.fit(X_train, X_train, epochs=epochs,
                    validation_data=(X_valid, X_valid),
                    callbacks=callbacks)
    denoise_conv_ae.evaluate(X_test, X_test)
    keras.models.save_model(denoise_conv_ae, f"{base_path}/saved_models/denoise_convolutional_autoencoder.keras")

def train_saprse_convolutional_autoencoder(X_train, X_valid, X_test, epochs=20, base_path:str="."):
    ...
    callbacks = get_default_callbacks(model_name="sparse_conv_ae", ckpt_path=f"{base_path}/ckpt", tensorboard_path=f"{base_path}/runs", csv_log_dir=f"{base_path}/logs")

    sparse_conv_ae.fit(X_train, X_train, epochs=epochs,
                    validation_data=(X_valid, X_valid),
                    callbacks=callbacks)
    sparse_conv_ae.evaluate(X_test, X_test)
    keras.models.save_model(sparse_conv_ae, f"{base_path}/saved_models/sparse_convolutional_autoencoder.keras")
